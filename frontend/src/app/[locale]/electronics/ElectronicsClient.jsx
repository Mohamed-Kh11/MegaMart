...
export default function ElectronicsClient({ locale }) {
  ...
  useEffect(() => {
    const fetchElectronics = async () => {
      try {
        ...
        const normalizedCategory = normalize(category || "electronics");
        ...
        const filteredElectronics = all.filter((p) => {
        ...
        setFiltered(filteredElectronics);
      } catch (err) {
  ...
  const handleAddToCart = async (product) => {
    ...
    try {
      // ✅ Get the authenticated user via JWT cookie
      const { data: me } = await api.get("/users/me", { withCredentials: true });

      if (me?._id) {
        // ✅ Fetch existing cart from the response
        ...
        // ✅ Save cart to server (JWT-protected)
        await api.put(
          "/users/me/cart",
          { cart: updatedCart },
      ...
      console.error("❌ Failed to update DB cart:", err.message);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center space-y-4">
        <h2 className="text-2xl font-semibold text-[#a28533] dark:text-amber-400 capitalize">
          {category?.replace(/-/g, " ") || "Electronics"}
        </h2>
    ...
    >
      ...
      <div className="flex items-center justify-center gap-4 mb-10 px-6">
        ...
        <h2 className="text-3xl font-extrabold text-[#a28533] dark:text-amber-400 whitespace-nowrap capitalize">
          {category?.replace(/-/g, " ") || t("electronics")}
        </h2>
        <div className="flex-1 max-w-[100px] h-[2px] bg-[#a28533] dark:bg-amber-400 rounded-full"></div>
      </div>
          ...
          filtered.map((product) => {
            ...
            return (
              ...
              >
                ...
                {/* 🛒 Buttons (always same distance from price) */}
