...
export default function GroceriesClient() {
  ...
  useEffect(() => {
    const fetchGroceries = async () => {
      try {
        ...
        const normalizedCategory = normalize(category || "groceries");
  ...
  useEffect(() => {
    ...
    // price filter uses base price (keep same behavior as other pages)
  ...
  const handleAddToCart = async (product) => {
    ...
    try {
      ...
      console.warn("⚠️ Falling back to local cart:", err.message);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center space-y-4">
        <h2 className="text-2xl font-semibold text-[#a28533] dark:text-amber-400 capitalize">
          {category?.replace(/-/g, " ") || t("groceries")}
        </h2>
